...
**Status:** Queued (M001 just completed)

## Current State

**Shipped:** v1.2 Production Readiness (2026-03-17, M001 complete)

The full stack is production-ready: indexer v2 with 6-step pipeline, 4 publishable npm packages at v1.1.0, production Docker Compose with monitoring stack, database backup automation, and operational tooling.
...
- **Production Docker Compose** — 10 services (indexer, postgres, hasura, loki, prometheus, alloy, cadvisor, grafana, backup)
- **Monitoring** — Grafana dashboard with 23 panels covering pipeline latency, entity throughput, verification health, metadata fetch, error analysis, container metrics
- **Database operations** — Automated cron backup, integrity verification, documented recovery runbook
...
- ✓ Database backups + recovery procedure — v1.2
...
All milestone work merged through pull requests targeting `refactor/indexer-v2-react`. Feature branches named `feat/<plan-description>`, PRs always target the integration branch.
...
| Block ordering with composite indexes | Deterministic pagination across all domains | ✓ Good — v1.2 |
| pg_dump over WAL archiving | Blockchain data re-derivable, fast recovery | ✓ Good — v1.2 |
| Entity registry with runtime validation | Compile-time + runtime type safety on BatchContext | ✓ Good — v1.2 |
...
_Last updated: 2026-03-17 after M001 completed_
