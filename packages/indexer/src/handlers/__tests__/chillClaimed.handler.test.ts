/**
 * Unit tests for ChillClaimed handler.
 *
...
import ChillClaimedHandler from '../chillwhales/chillClaimed.handler';
...
// Mock Store helper
// ---------------------------------------------------------------------------
function createMockStore(existingChillClaimed: ChillClaimed[] = []): Store {
  return {
    findBy: vi.fn((entityClass: unknown, where: { value: boolean }) => {
      if (entityClass === ChillClaimed) {
        return Promise.resolve(existingChillClaimed.filter((e) => e.value === where.value));
      }
...
describe('ChillClaimedHandler - Phase 1 (Mint Detection)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe('creates entities with value=false for mint transfers', () => {
    it('creates ChillClaimed entity for Chillwhale mint (from zero address)', async () => {
    ...
    it('sets FK fields to null initially', async () => {
      ...
      const addedEntity = batchCtx.addEntity.mock.calls[0][2] as ChillClaimed;
...
describe('ChillClaimedHandler - Phase 2 (On-chain Verification)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe('queries unclaimed entities when isHead=true', () => {
    it('queries database for unclaimed ChillClaimed entities (value=false)', async () => {
      ...
      expect(store.findBy).toHaveBeenCalledWith(ChillClaimed, { value: false });
    });
