...
/**
 * GraphQL subscription document for real-time issued asset updates.
 *
 * Mirrors `GetIssuedAssetsDocument` field selections and `@include` directives exactly.
 * Differences from query: `subscription` keyword, no `$offset`, no `_aggregate`.
 */
export const IssuedAssetSubscriptionDocument = `
