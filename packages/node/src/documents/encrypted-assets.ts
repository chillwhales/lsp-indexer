...
/**
 * GraphQL subscription document for real-time encrypted asset updates.
 *
 * Mirrors `GetEncryptedAssetsDocument` field selections and `@include` directives exactly.
 * Differences from query: `subscription` keyword, no `$offset`, no `_aggregate`.
 */
export const EncryptedAssetSubscriptionDocument = `
