...
`);
/**
 * GraphQL subscription document for real-time NFT updates.
 *
 * Mirrors `GetNftsDocument` field selections and `@include` directives exactly.
 * Differences from query: `subscription` keyword, no `$offset`, no `_aggregate`.
 */
export const NftSubscriptionDocument = `
