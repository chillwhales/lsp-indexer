...
`);
/**
 * GraphQL subscription document for real-time follower updates.
 *
 * Mirrors `GetFollowersDocument` field selections and `@include` directives exactly.
 * Differences from query: `subscription` keyword, no `$offset`, no `_aggregate`.
 */
export const FollowerSubscriptionDocument = `
