...
`);
/**
 * GraphQL subscription document for real-time owned token updates.
 *
 * Mirrors `GetOwnedTokensDocument` field selections and `@include` directives exactly.
 * Differences from query: `subscription` keyword, no `$offset`, no `_aggregate`.
 */
export const OwnedTokenSubscriptionDocument = `
