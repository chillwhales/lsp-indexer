...
plan: 01
subsystem: infra
tags: [grafana, loki, alloy, prometheus, cadvisor, docker-compose, monitoring]

requires:
  - phase: 18-docker-image
    provides: Production docker-compose with 4 services (postgres, indexer, hasura, data-connector-agent)
provides:
...
affects: [20-02-dashboards, monitoring, production-deployment]

tech-stack:
  added: [grafana 11.4, loki 3.4, alloy 1.8, prometheus 3.2, cadvisor 0.51]
  patterns: [grafana-provisioning, alloy-river-syntax, remote-write, docker-log-collection]
...
patterns-established:
  - "Monitoring config files live in docker/{service}/ directories"
  - "Datasource UIDs (loki, prometheus) are referenced by dashboard JSON"
  - "Alloy labels: container_name, compose_service, compose_project"
