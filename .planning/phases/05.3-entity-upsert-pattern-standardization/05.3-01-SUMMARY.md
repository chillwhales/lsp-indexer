...
plan: 01
subsystem: core
tags: [typeorm, batch-context, entity-resolution, handler-helpers]

# Dependency graph
requires:
  - phase: 05.2-lsp4-base-uri-count-parity
    provides: Handler implementations that will be refactored in subsequent plans
provides:
...
tech-stack:
  added: []
  patterns:
    - 'resolveEntity/resolveEntities pattern: batch → DB → null/Map'
    - 'Spread pattern foundation: ...existing preserves all fields including FKs'
