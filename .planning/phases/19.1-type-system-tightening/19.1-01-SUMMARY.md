...
plan: 01
subsystem: indexer
tags: [typescript, type-safety, batchcontext, getEntities, explicit-casts]

# Dependency graph
requires:
  - phase: 19-block-ordering
    provides: Widened Entity base type with block ordering fields
provides:
...
tech-stack:
  added: []
  patterns:
    - "Explicit cast pattern: getEntities(key) as Map<string, ConcreteType>"
    - "Test entity pattern: extract to variable, cast as Entity for addEntity()"
