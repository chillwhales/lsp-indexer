...
plan: 01
subsystem: infra
tags: [pino, pino-roll, structured-logging, subsquid-logger, vitest]

# Dependency graph
requires:
  - phase: none
    provides: Standalone infrastructure module (no prior phase dependency)
provides:
...
affects: [02-04-PLAN.md (replace JSON.stringify logging), phase-3 (metadata handlers use logging), phase-4 (pipeline integration)]

# Tech tracking
tech-stack:
  added: [pino@^9.6.0, pino-roll@^1.1.0, vitest@^3.0.0, @subsquid/logger@^1.4.0]
  patterns: [dual-output logging (Subsquid + pino), singleton file logger, child logger factory for pipeline steps]
