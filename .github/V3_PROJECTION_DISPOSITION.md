...
- Typed UP and digital-asset rows are created only after successful verification. Invalid optional
  ...
  moving away from LSP8 clears the collection-only fields and dependent NFT, token-ownership, and
  extension rows without deleting raw facts or values.
...
| `lsp4Creators`             | Length/index/map events merge into one row per creator; shrinks delete stale rows and profile reverification refreshes the derived flag.    | `creators`, `data_values`                                                  |
...
| `lsp6Controllers`          | Length/index/maps merge into one row; array removal preserves independent permission maps, and CompactBytesArray values become JSON arrays. | `controllers`, `data_values`                                               |
...
| `orbLevel`                 | Network-gated mint defaults and packed token-data reduction; empty or short values clear both derived fields.                               | `chillwhales_nfts.level`, `cooldown_expiry`                                |
