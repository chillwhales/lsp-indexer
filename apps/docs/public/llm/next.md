...
| `useFollowedByMyFollows`         | `getFollowedByMyFollows` | Profiles followed by `targetAddress` that also follow `myAddress` |
...
function MutualFollows({ addressA, addressB }: { addressA: string; addressB: string }) {
  const { profiles, totalCount, isLoading } = useMutualFollows(addressA, addressB, {
