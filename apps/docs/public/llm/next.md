...
```bash
npm install @lsp-indexer/next@^3 @tanstack/react-query
```
...
The v3 Node helpers expose `getServerNetwork()` and `getClientNetwork()`, but Next hooks and server
actions still require `network` in their parameter object. This makes the selected chain visible at
the call site and prevents a request from silently following a deployment default.
...
| Env vars            | `NEXT_PUBLIC_INDEXER_URL` + network | `INDEXER_URL` + network                |
...
Everything else stays the same—filters, sorting, include fields, and infinite scroll. Every hook
parameter object requires `network`; it is forwarded through the server action and included in the
cache key.
...
function MyComponent() {
  ...
  const { profile } = useProfile({ network, address: '0x...' });
  const { digitalAssets } = useDigitalAssets({
    network,
    filter: { tokenType: 'NFT' },
    limit: 10,
  });
  const { nfts, fetchNextPage } = useInfiniteNfts({ network, pageSize: 20 });
  // ...
}
```
...
export default async function Page() {
  const profile = await getProfile({ network: 'lukso-mainnet', address: '0x...' });
...
`useEncryptedAssetsBatch` fetches multiple encrypted assets by `(address, contentId, revision)` tuples via the `getEncryptedAssetsBatch` server action. The action transparently pages Hasura when duplicate immutable revisions require another batch.
...
const include: ProfileInclude = { name: true, profileImage: true };
...
export default async function Page() {
  const { profiles } = await getMutualFollows({
    network: 'lukso-mainnet',
    addressA: '0x...',
    addressB: '0x...',
    limit: 10,
...
export default async function CollectionPage({ address }: { address: string }) {
  const { attributes, totalCount } = await getCollectionAttributes({
    network: 'lukso-mainnet',
...
const sort: NftSort = { field: 'tokenId', direction: 'asc', nulls: 'last' };
```

Supported v3 sort fields are `newest`, `oldest`, `tokenId`, and `formattedTokenId`. The legacy
`score` input is rejected with a field-level `VALIDATION_FAILED` error instead of being ignored.
...
| `INDEXER_NETWORK`             | V3 helpers          | Exact server-side network slug     |
| `NEXT_PUBLIC_INDEXER_NETWORK` | Browser hooks       | Exact browser-visible network slug |
