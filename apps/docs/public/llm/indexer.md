...
| `DATABASE_UNFINALIZED_BLOCKS_RETENTION` | No       | At least `1000` and greater than network finality           |
...
columns across chains; existing definitions must match their canonical labels and ordering. The
`api` schema excludes migration history, cursors, metadata jobs, snapshots, functions, and triggers.
Unexpected API relations abort migration, and the API reader receives `SELECT` only on the
enumerated public views.

Create runtime login roles through your PostgreSQL provisioning system, then grant them during the
one-shot migration. Each login must be unique to one network and must not have elevated PostgreSQL
capabilities. Deterministic schema owner and writer roles remain capability-limited
`NOLOGIN NOINHERIT` roles without direct or transitive memberships in other roles:
