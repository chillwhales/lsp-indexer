...
outside the stored range does not advance the watermark. When a live source reports finality ahead
of a historical backfill cursor, the target records the processed cursor and its hash as the highest
...
schema is current. If a migration is rejected, it attempts to restore the views before returning the
original error. Migration and startup also inventory the migration table, cursor, and every expected
chain table and require the deterministic writer role to own each one.
