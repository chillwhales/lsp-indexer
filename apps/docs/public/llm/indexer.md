...
rejects concurrent migration commands.
...
columns across chains. The `api` schema excludes migration history, cursors, metadata jobs,
snapshots, functions, and triggers.
...
V3 therefore rejects pending schema migrations while any rollback snapshot contains rows. During
alpha, use a fresh rebuild unless the repository owner approves a tested snapshot-preserving
procedure.
