...
Later domain decoders can therefore select contract-scoped capabilities without scattering chain
allowlists through event modules.
...
the source returns both inclusive range boundaries, so a partial response is not reported as a
