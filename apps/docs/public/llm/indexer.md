...
Only the current migration admin may reach the API owner role. Revoke the previous membership before
rotating either credential. Because a PostgreSQL session can `RESET ROLE`, migration and startup
also reject direct ACLs, object ownership, default ACLs, and policy references held by the underlying
runtime login; the only permitted direct database ACL is non-grantable `CONNECT` on the current
database. The writer itself may own or receive privileges only inside its assigned chain schema.
