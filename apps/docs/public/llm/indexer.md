...
stale privilege aborts migration.
...
previous membership before rotating either credential. Programmatic migration calls also reject
duplicate network identities or schemas, and an existing chain schema must contain only its expected
singleton `network_config` identity:
