...
`FOR UPDATE SKIP LOCKED`, and expiring processing leases. Claims, lease expiry, and retries use the
PostgreSQL transaction clock, so clock skew between worker replicas cannot steal or strand leases.
Multiple replicas may drain the same network queue. Shutdown signals wake idle workers immediately
and close their metrics server and database pool after in-flight work settles. Each worker reloads
