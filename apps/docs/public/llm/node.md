...
Framework adapters can call `fetchV3Domain(url, domain, params)` to retain the exact domain-specific
field and result types without copying the `v3Api` dispatch table.
...
`v3Keys` and every familiar key factory include `v3` plus the exact network. Uniform infinite keys
also include domain, filter, sort, and page size. The same address on two chains therefore produces
different keys. Generated GraphQL documents and the old public
