...
# Optional — for server-side subscriptions via WS proxy
# INDEXER_WS_URL=ws://localhost:8080/v1/graphql
```

### Both modes

Set all four variables to enable toggling between client and server mode:
...
Replace `Profile` with any domain: `DigitalAsset`, `NFT`, `OwnedAsset`, `Creator`, `Follow`, etc.
