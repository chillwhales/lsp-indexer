...
> want real-time subscriptions, use `@lsp-indexer/react` subscription hooks and point the provider
> at the WS proxy from `@lsp-indexer/next/server`. This keeps the Hasura URL hidden from the
> browser while enabling real-time updates:
>
> ```tsx
> <IndexerSubscriptionProvider wsUrl="ws://localhost:4000">
> ```
>
