...
```ts
import { fetchProfile } from '@lsp-indexer/node';
...
profileKeys.list({ filter, sort, limit }); // ['profiles', 'list', { filter, sort, limit }]
profileKeys.details(); // ['profiles', 'detail']
profileKeys.detail({ address }); // ['profiles', 'detail', { address }]
```
...
Low-level WebSocket subscription client for real-time data:
...
subscription.on('data', (data) => { /* handle update */ });
subscription.on('error', (err) => { /* handle error */ });
```

In practice, you'll use the higher-level `useSubscription` hook from `@lsp-indexer/react` or
`@lsp-indexer/next` instead of the raw client.
