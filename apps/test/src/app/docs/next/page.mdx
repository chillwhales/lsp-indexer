...
To use both server actions and subscriptions, set up the React subscription provider pointed at
the WS proxy so the Hasura URL stays hidden from the browser:
...
export function Providers({ children }: { children: React.ReactNode }) {
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <IndexerSubscriptionProvider wsUrl={`ws://localhost:${wsProxyPort}`}>
        {children}
...
> **Tip:** In production, replace `ws://localhost:4000` with the deployed WS proxy URL.
> The proxy keeps the real Hasura WebSocket URL hidden from the browser.
