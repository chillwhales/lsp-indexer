...
```bash
npm install @lsp-indexer/next @lsp-indexer/react @lsp-indexer/types @lsp-indexer/node @tanstack/react-query
```
...
> **Important:** You may need to forward these env vars via `next.config.ts` for them to be
> available in server actions from transpiled workspace packages:
>
> ```ts
> // next.config.ts
> const nextConfig = {
...
```ts
// ws-proxy.ts
import { createIndexerProxy } from '@lsp-indexer/next/server';

const proxy = createIndexerProxy({
